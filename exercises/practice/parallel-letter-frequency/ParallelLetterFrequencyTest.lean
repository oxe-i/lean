...
def parallelLetterFrequencyTests : TestSuite :=
  (TestSuite.empty "ParallelLetterFrequency")
  |>.addTest "no texts" (do
      let start ← IO.monoNanosNow
      let actual ← (ParallelLetterFrequency.calculateFrequencies [])
      let stop ← IO.monoNanosNow
      let expected : Std.TreeMap Char Nat := {}
      IO.println s!"Execution time for 'no texts': {stop - start} ns"
      return assertEqual expected actual)
  |>.addTest "one text with one letter" (do
      let start ← IO.monoNanosNow
      let actual ← (ParallelLetterFrequency.calculateFrequencies ["a"])
      let stop ← IO.monoNanosNow
      let expected : Std.TreeMap Char Nat := Std.TreeMap.ofList [('a', 1)]
      IO.println s!"Execution time for 'one text with one letter': {stop - start} ns"
      return assertEqual expected actual)
  |>.addTest "one text with multiple letters" (do
      let start ← IO.monoNanosNow
      let actual ← (ParallelLetterFrequency.calculateFrequencies ["bbcccd"])
      let stop ← IO.monoNanosNow
      let expected : Std.TreeMap Char Nat := Std.TreeMap.ofList [('b', 2), ('c', 3), ('d', 1)]
      IO.println s!"Execution time for 'one text with multiple letters': {stop - start} ns"
      return assertEqual expected actual)
  |>.addTest "two texts with one letter" (do
      let start ← IO.monoNanosNow
      let actual ← (ParallelLetterFrequency.calculateFrequencies ["e","f"])
      let stop ← IO.monoNanosNow
      let expected : Std.TreeMap Char Nat := Std.TreeMap.ofList [('e', 1), ('f', 1)]
      IO.println s!"Execution time for 'two texts with one letter': {stop - start} ns"
      return assertEqual expected actual)
  |>.addTest "two texts with multiple letters" (do
      let start ← IO.monoNanosNow
      let actual ← (ParallelLetterFrequency.calculateFrequencies ["ggh","hhi"])
      let stop ← IO.monoNanosNow
      let expected : Std.TreeMap Char Nat := Std.TreeMap.ofList [('g', 2), ('h', 3), ('i', 1)]
      IO.println s!"Execution time for 'two texts with multiple letters': {stop - start} ns"
      return assertEqual expected actual)
  |>.addTest "ignore letter casing" (do
      let start ← IO.monoNanosNow
      let actual ← (ParallelLetterFrequency.calculateFrequencies ["m","M"])
      let stop ← IO.monoNanosNow
      let expected : Std.TreeMap Char Nat := Std.TreeMap.ofList [('m', 2)]
      IO.println s!"Execution time for 'ignore letter casing': {stop - start} ns"
      return assertEqual expected actual)
  |>.addTest "ignore whitespace" (do
      let start ← IO.monoNanosNow
      let actual ← (ParallelLetterFrequency.calculateFrequencies ["   ","\u0009","\r\n"])
      let stop ← IO.monoNanosNow
      let expected : Std.TreeMap Char Nat := {}
      IO.println s!"Execution time for 'ignore whitespace': {stop - start} ns"
      return assertEqual expected actual)
